...
for iSNR = 1:length(EbN0dB)
    ...
    while (bitErr < nbrErreur && pqtNbr*K < nbrBitMax)
        ...
        [c, s_f] = cc_encode(u, trellis, s_i, closed); % Encodage
        ...
        u_rec = viterbi_decode(Lc, trellis, s_i, closed); % Décodage
...
fprintf(line      );
%%
save('structures_MONTECARLO/MC_2_3.mat','EbN0dB','TEB', 'TEP', 'R', 'K', 'N', 'Pb_u', 'Pe_u')
